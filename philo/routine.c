...
/*   Updated: 2025/01/09 15:08:08 by jparnahy         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void *philo_life(void *philo_ptr)
{
    ...
    while (!philo->table->dead)
    {
        ...
        if (philo->table->dead)
            break;
        print_status(philo, PHILO_THINK);
    }
    return (NULL);
}
...
void    *maitre(void *philo_ptr)
{
    ...
    {
        pthread_mutex_lock(&philo->meal_lock);
        if (get_time() >= philo->the_death && !philo->eating)
        {
            print_status(philo, PHILO_DIED);
            philo->table->dead = 1;
        }
...
void    *the_routine(void *philo_ptr)
{
    ...
    philo = (t_philo *)philo_ptr;
    if (pthread_create(&philo->thread, NULL, &maitre, philo))
        return ((void *)1);
